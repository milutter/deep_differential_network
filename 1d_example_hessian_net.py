...
if __name__ == "__main__":
    ...
    filename = f"1d_{hyper['activation']}_diff_hessian_net"
    ...
    for i in range(2):
        ...
        err_d2yd2x = 1. / float(x_test.shape[0]) * np.sum((d2yd2x_test - plot_d2yd2x_test[i]) ** 2)
    ...
    ax0.plot(x_test, plot_y_test[0][:, 0], color="r")
    ax0.plot(x_test, plot_y_test[1][:, 0], color="m")

    ax1.plot(x_test, plot_dydx_test[0][:, 0], color="r")
    ax1.plot(x_test, plot_dydx_test[1][:, 0], color="m")

    ax2.plot(x_test, plot_d2yd2x_test[0][:, 0], color="r")
    ax2.plot(x_test, plot_d2yd2x_test[1][:, 0], color="m")
