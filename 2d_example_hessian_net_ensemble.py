...
if __name__ == "__main__":
    ...
    filename = f"2d_{hyper['activation']}_diff_hessian_net_ensemble"
    ...
    for i in range(2):
            ...
            for x_i, y_i, dydx_i in mem:
                ...
                if i == 0:
                    ...
                    l2_err = torch.sum((y_i_hat - y_i.view(1, -1, y_i.shape[1], 1)) ** 2, dim=[0, 2]) + \
                             torch.sum((dydx_i_hat.transpose(2, 3) - dydx_i.view(1, -1, dydx_i.shape[1], 1)) ** 2, dim=[0, 2])
        ...
        err_y = 1. / n_norm * np.sum((y_hat[:, :, 0] - y_test) ** 2)
        err_dydx = 1. / n_norm * np.sum((dydx_hat - dydx_test) ** 2)
        err_d2yd2x = 1. / n_norm * np.sum((d2yd2x_hat - d2yd2x_test) ** 2)
        ...
        t0_jac = time.perf_counter()
        dydx_autograd[i] = jacobian(f_diff_net, x_torch, create_graph=False, v1=False).detach().cpu().numpy().squeeze().transpose((1, 0, 2))
        t_rev_jac = time.perf_counter() - t0_jac

        t0_hes = time.perf_counter()
        d2yd2x_autograd[i] = jacobian(dfdx_diff_net, x_torch, create_graph=False, v1=False).detach().cpu().numpy().squeeze().transpose((1, 0, 2, 3))
        t_rev_hes = time.perf_counter() - t0_hes

        n_norm = (float(x_test.shape[0]) * hyper['n_ensemble'])
        err_dydx = 1. / n_norm * np.sum((dydx_test - plot_dydx_test[i]) ** 2)
        err_d2yd2x = 1. / n_norm * np.sum((d2yd2x_test - d2yd2x_hat) ** 2)
    ...
    cset0 = ax0.contourf(x0_mat, x1_mat, np.mean(plot_y_test[0], axis=0).reshape(x0_mat.shape),
                        levels=n_levels, norm=norm, cmap=cm.get_cmap(cmap, n_levels))

    ax1 = shape_axis(fig.add_subplot(3, 6, 8))
    cset1 = ax1.contourf(x0_mat, x1_mat, np.mean(plot_dydx_test[0], axis=0)[:, 0].reshape(x0_mat.shape),
                        levels=n_levels, norm=norm, cmap=cm.get_cmap(cmap, n_levels))

    ax2 = shape_axis(fig.add_subplot(3, 6, 9))
    cset2 = ax2.contourf(x0_mat, x1_mat, np.mean(plot_dydx_test[0], axis=0)[:, 1].reshape(x0_mat.shape),
                        levels=n_levels, norm=norm, cmap=cm.get_cmap(cmap, n_levels))

    ax3 = shape_axis(fig.add_subplot(3, 6, 10))
    cset3 = ax3.contourf(x0_mat, x1_mat, np.mean(d2yd2x_autograd[0], axis=0)[:, 0, 0].reshape(x0_mat.shape),
                         levels=n_levels, norm=norm, cmap=cm.get_cmap(cmap, n_levels))

    ax4 = shape_axis(fig.add_subplot(3, 6, 11))
    cset4 = ax4.contourf(x0_mat, x1_mat, np.mean(d2yd2x_autograd[0], axis=0)[:, 1, 1].reshape(x0_mat.shape),
                         levels=n_levels, norm=norm, cmap=cm.get_cmap(cmap, n_levels))

    ax5 = shape_axis(fig.add_subplot(3, 6, 12))
    cset5 = ax5.contourf(x0_mat, x1_mat, np.mean(d2yd2x_autograd[0], axis=0)[:, 1, 0].reshape(x0_mat.shape),
                         levels=n_levels, norm=norm, cmap=cm.get_cmap(cmap, n_levels))
    ...
    cset0 = ax0.contourf(x0_mat, x1_mat, np.mean(plot_y_test[1], axis=0).reshape(x0_mat.shape),
                        levels=n_levels, norm=norm, cmap=cm.get_cmap(cmap, n_levels))

    ax1 = shape_axis(fig.add_subplot(3, 6, 14))
    cset1 = ax1.contourf(x0_mat, x1_mat, np.mean(plot_dydx_test[1], axis=0)[:, 0].reshape(x0_mat.shape),
                         levels=n_levels, norm=norm, cmap=cm.get_cmap(cmap, n_levels))

    ax2 = shape_axis(fig.add_subplot(3, 6, 15))
    cset2 = ax2.contourf(x0_mat, x1_mat, np.mean(plot_dydx_test[1], axis=0)[:, 1].reshape(x0_mat.shape),
                        levels=n_levels, norm=norm, cmap=cm.get_cmap(cmap, n_levels))

    ax3 = shape_axis(fig.add_subplot(3, 6, 16))
    cset3 = ax3.contourf(x0_mat, x1_mat, np.mean(d2yd2x_autograd[1], axis=0)[:, 0, 0].reshape(x0_mat.shape),
                         levels=n_levels, norm=norm, cmap=cm.get_cmap(cmap, n_levels))

    ax4 = shape_axis(fig.add_subplot(3, 6, 17))
    cset4 = ax4.contourf(x0_mat, x1_mat, np.mean(d2yd2x_autograd[1], axis=0)[:, 1, 1].reshape(x0_mat.shape),
                         levels=n_levels, norm=norm, cmap=cm.get_cmap(cmap, n_levels))

    ax5 = shape_axis(fig.add_subplot(3, 6, 18))
    cset5 = ax5.contourf(x0_mat, x1_mat, np.mean(d2yd2x_autograd[1], axis=0)[:, 1, 0].reshape(x0_mat.shape),
                        levels=n_levels, norm=norm, cmap=cm.get_cmap(cmap, n_levels))
