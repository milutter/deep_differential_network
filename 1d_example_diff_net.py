...
from deep_differential_network.differential_network import DifferentialNetwork
...
if __name__ == "__main__":
    ...
    filename = f"1d_{hyper['activation']}_diff_net"
    ...
    x_train = np.random.uniform(-np.pi, np.pi, n_train_samples)[:, np.newaxis]
        ...
        while epoch_i < max_epoch and not LOAD_MODEL:
            ...
            for x_i, y_i, dydx_i in mem:
                ...
                if i == 0:
                    ...
                    l2_err = torch.sum((y_i_hat - y_i) ** 2, dim=1) + \
                ...
                l_mean = torch.mean(l2_err)
                l_var = torch.var(l2_err)
    ...
    ax0.plot(x_test, plot_y_test[0], color="r")
    ax0.plot(x_test, plot_y_test[1], color="m")

    ax1.plot(x_test, plot_dydx_test[0], color="r")
    ax1.plot(x_test, plot_dydx_test[1], color="m")

    ax2.plot(x_test, d2yd2x_autograd[0], color="r")
    ax2.plot(x_test, d2yd2x_autograd[1], color="m")
